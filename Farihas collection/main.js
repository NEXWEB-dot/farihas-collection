document.addEventListener('DOMContentLoaded', () => {
    ...
    function openCartPanel() {
        ...
        validateCartStockMain();
    }

    async function validateCartStockMain() {
        if (!cart || !cart.length) return;
        const ids = cart.map(item => item.id).filter(id => id);
        ...
        try {
            ...
            const query = `*[_type in ["shoe", "clearanceSale"] && _id in ${JSON.stringify(ids)}]{ _id, name, soldOut, sizes }`;
            ...
            if (cartModified) {
                ...
                saveCart();
                renderCartPanel();
                alert(`Sorry, the following item(s) have just sold out and were removed from your cart:\n\n- ${removedNames.join('\n- ')}`);
            }
        } catch (e) {
