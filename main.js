document.addEventListener('DOMContentLoaded', () => {
    ...
    // ===== SANITY HELPERS =====
    // apicdn (not api) routes reads through Sanity's cached edge CDN instead of
    // the live API on every request — used by every Sanity fetch in this file.
    function sanityCdnUrl(projectId, dataset, apiVersion, query) {
        return `https://${projectId}.apicdn.sanity.io/v${apiVersion}/data/query/${dataset}?query=${encodeURIComponent(query)}`;
    }
    // Appends Sanity's image transform params so the CDN serves a resized,
    // compressed, next-gen-format image instead of the full original upload.
    function sanityImg(url, width, quality) {
        if (!url) return url;
        const w = width || 600;
        const q = quality || 75;
        const sep = url.includes('?') ? '&' : '?';
        return `${url}${sep}w=${w}&auto=format&q=${q}`;
    }
    ...
    function createSearchOverlay() {
        ...
        input.addEventListener('input', () => {
            // Debounce: wait until the user pauses typing instead of firing a
            // fresh Sanity request on every single keystroke. This was the
            // single biggest source of bandwidth/API usage in search — typing
            // a 5-letter word used to mean 5 separate live queries.
            clearTimeout(input._searchDebounce);
            input._searchDebounce = setTimeout(performSearch, 350);
        });
    }
    ...
    async function performSearch() {
        ...
        // Skip firing on 1-2 characters — too broad to be useful and just
        // burns requests before the user has typed anything meaningful.
        if (query.length < 3) {
            resultsContainer.innerHTML = '<p class="search-hint">Keep typing...</p>';
            return;
        }
        ...
        try {
            ...
            const url = sanityCdnUrl(SANITY_PROJECT_ID, SANITY_DATASET, SANITY_API_VERSION, sanityQuery);
            ...
            matches.forEach(m => {
                ...
                const image = sanityImg(m.image, 200) || '';
                const brand = m.brand || m.tag || '';
                const params = new URLSearchParams({ id: m._id, name, price, image: sanityImg(m.image, 900) || '' });
    ...
    async function loadGlobalBanner() {
        try {
            ...
            const url = sanityCdnUrl(SANITY_PROJECT_ID, SANITY_DATASET, SANITY_API_VERSION, query);
            ...
            if (settings) {
                ...
                if (img1 && settings.promoBanner1) img1.src = sanityImg(settings.promoBanner1, 1600);
                
                const img2 = document.getElementById('promoBannerImg2');
                if (img2 && settings.promoBanner2) img2.src = sanityImg(settings.promoBanner2, 1600);
